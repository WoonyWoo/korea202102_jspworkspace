...
public class DeleteServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result!=0) { //삭제가 되었다면 파일도 삭제하자!!
			//파일을 삭제하라면, 결국 파일을 제어해야 한다..자바에서 파일을 제어하기 위해 제공되는 객체가 바로 File 클래스
			//개발자가 실제 이 웹어플리케이션이 시스템의 어느 경로에 있는지 알 필요없이, 어플리케이션의 루트를 기준으로 경로를 표현하려면
			//개발자가 아닌 시스템에 의해 즉 프로그래밍적으로 경로를 얻는 방법이 있다!! 그러기 위해서는 현재 웹 어플리케이션의 정보를 가진
			//객체를 이용하면 된다..
